...
const domain = process.env?.APP_DOTNS_DOMAIN ?? "playground-template";
const label = domain.toLowerCase().replace(/\.dot$/, "");

export default defineConfig({
  domain: `${label}.dot`,
  displayName: "Polkadot Playground",
  description:
    "Minimal React + Vite + TypeScript template wired to the Host API for product-account access from a Polkadot host — a starting point for building Polkadot dapps.",
