...
// Demo chain read: subscribe to devnet Asset Hub's current block number through
...
// devnet_asset_hub keeps the build to a single metadata chunk. The host must
...
export function useChainBlock(): ChainBlockState {
    ...
    useEffect(() => {
        ...
        const genesis = devnet_asset_hub.genesis as `0x${string}`;
        ...
        (async () => {
            ...
            if (supported.ok && !supported.value) {
                throw new Error(
                    `Host does not support devnet Asset Hub (genesis ${genesis}).`,
                );
            }
...
// PGAS balance, as held on devnet Asset Hub (Paseo). PGAS is the Hub's gas/fee token —
...
// One-shot on-chain reads for the product account on devnet Asset Hub (Paseo): whether
